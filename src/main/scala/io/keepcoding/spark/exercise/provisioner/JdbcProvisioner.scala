...
object JdbcProvisioner {
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      println("Creando la tabla bytes (timestamp TIMESTAMP WITHOUT TIME ZONE, id TEXT, value BIGINT, type TEXT)")
      statement.execute("CREATE TABLE IF NOT EXISTS bytes (timestamp TIMESTAMP WITHOUT TIME ZONE, id TEXT, value BIGINT, type TEXT)")

      println("Creando la tabla bytes_hourly (timestamp TIMESTAMP WITHOUT TIME ZONE, id TEXT, value BIGINT, type TEXT)")
      statement.execute("CREATE TABLE IF NOT EXISTS bytes_hourly (timestamp TIMESTAMP WITHOUT TIME ZONE, id TEXT, value BIGINT, type TEXT)")
